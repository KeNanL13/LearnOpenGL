...
void MyOpenGLWidget::initializeGL()
{
    ...
    //简单shader
    singleShader.addShaderFromSourceFile(QOpenGLShader::Vertex,":/GLSL/vertexbasic.vert");
    singleShader.addShaderFromSourceFile(QOpenGLShader::Fragment,":/GLSL/shadersinglecolor.frag");
    singleShader.link();
    //物体的shaderprogram
    ...
    modelSingle.scale(1.1f);
    modelTranslate.translate(3,0,3);
    shaderProgram.setUniformValue("projection",projection);
    shaderProgram.setUniformValue("model",model);
    shaderProgram.setUniformValue("texture_diffuse",0);

    singleShader.setUniformValue("projection",projection);
    singleShader.setUniformValue("model",modelSingle);
...
//    shaderProgram.setUniformValue("cameraPos",camera.getCameraPos());
    ...
    singleShader.setUniformValue("projection",projection);
    singleShader.setUniformValue("model",modelSingle);
    singleShader.setUniformValue("view",camera.getView());
    switch (m_Shape) {
    case Rect:
        shaderProgram.bind();
        //画地面
        ...
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES,0,36);
        singleShader.setUniformValue("model",modelTranslate*modelSingle);
        glDrawArrays(GL_TRIANGLES,0,36);
        glStencilMask(0xFF);
        glEnable(GL_DEPTH_TEST);
